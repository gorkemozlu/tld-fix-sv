...
export VMS=$(KUBECONFIG=/.kube/config/admin.conf kubectl get vm -n $NS|grep $CLUSTER_NAME|awk '{print $1}')
for VM in $(KUBECONFIG=/.kube/config/admin.conf kubectl get vm -n $NS|grep $CLUSTER_NAME|awk '{print $1}')
do
    export VM_IP=$(KUBECONFIG=/.kube/config/admin.conf kubectl get vm $VM -n $NS -o jsonpath='{.status.vmIp}')
    export VM_SECRET=$(KUBECONFIG=/.kube/config/admin.conf kubectl get secret $CLUSTER_NAME-ssh-password -n $NS -o jsonpath='{.data.ssh-passwordkey}'|base64 -d)
